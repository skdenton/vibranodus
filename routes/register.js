...
exports.reset = function(req, res, next) {
    ...
    User.getByName(urluser, function(err, user) {
        ...
        if (user.uid) {
            ...
            var complete_string =
                user.substance + user.portal + user.pepper + nowtime
            ...
            bcrypt.compare(complete_string, hash, function(err, ress) {
                if (err) {
                    ...
                    if (req.params.hash && ress) {
                        ...
                        User.modifyPassword(urluser, newpass, function(
                            err,
                            answer
                        ) {
                            if (err) {
                                ...
                                res.send({
                                    moveon: '/login?login=' + data.username,
                                    errormsg:
...
exports.generatehash = function(req, res, next) {
    ...
    User.getByNameEmail(data.username, data.email, function(err, user) {
        ...
        if (user.uid) {
            ...
            var resetLink =
                '/reset/' +
                encodeURIComponent(user.substance) +
                '/' +
                nowtime.toString(36) +
                '/' +
                hash.replace(/\//g, 'slashslash')
                ...
                transporter.sendMail(mailOptions, function(err, ress) {
                    if (err) {
                        ...
                        res.send({
                            success: 'True',
                            errormsg:
                                'Sorry, but there was a problem sending a password reset link to your email.',
                        })
                    } else {
                        console.log('mail sent')
                        res.send({
                            errormsg:
                                'The recovery link has been sent to the e-mail associated with your username.',
                        })
                    }
                })
            }
            ...
            if (data.username && data.email) {
                res.send({
                    errormsg:
                        'We did not find this username / email combination. Try to enter either the username or the email and try again.',
                })
            } else if (data.username) {
                res.send({
                    errormsg:
                        "There's no record of this username in our database. Usernames are case-sensitive, so you can try again. Or enter only the email.",
                })
            } else if (data.email) {
                res.send({
                    errormsg:
                        "There's no record of this email in our database. Try submitting the form with just the username.",
                })
            }
        }
    })
       ...
       // Ok, now on to the account creation
         ...
         // As there's no invitation code or it's not right, let's proceed to create a subscription for the user
           ...
           if (data.invite.length == 0 && options.chargebee && options.chargebee.site && options.chargebee.api_key ) {
                    ...
                    res.send({
                        errormsg: 'Please, enter the correct invitation code. ',
                    })
                }
            }
        }
