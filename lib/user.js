...
User.prototype.hashPassword = function(fn) {
    ...
    bcrypt.genSalt(12, function(err, salt) {
        if (err) return fn(err)
        user.salt = salt
        bcrypt.hash(user.pepper, salt, function(err, hash) {
            if (err) return fn(err)
            user.pepper = hash
            fn()
        })
...
User.get = function(uid, fn) {
    ...
    dbneo.cypherQuery(userQuery, function(err, user) {
        ...
        if (user && Array.isArray(user.data) && user.data.length > 0) {
            var firstRow = user.data[0]
            if (firstRow && typeof firstRow === 'object' && !Array.isArray(firstRow)) {
                userData = firstRow
            }
        }

        fn(null, new User(userData))
    })
}
...
User.getIdNameEmail = function(name, email, fn) {
    ...
    dbneo.cypherQuery(idQuery, function(err, uid) {
        ...
        var data = Array.isArray(uid && uid.data) ? uid.data : []
        var firstRow = data.length > 0 ? data[0] : null
        var identifier = firstRow

        if (firstRow && typeof firstRow === 'object') {
            if (firstRow.uid) {
                identifier = firstRow.uid
            } else if (firstRow.id) {
                identifier = firstRow.id
            }
        }
