...
const dbneo = require('./db/neo4jClient')
...
Entry.getNodes = function(
    ...
    // PROFILE CALL db.index.fulltext.queryRelationships('to_context_fulltext','context:34b4a5b0-0dfa-11e9-98ed-7761a512a9c0')
    // YIELD relationship AS rel, startNode AS start, endNode AS end WITH DISTINCT rel, start, end RETURN DISTINCT start.uid AS source_id,
    // start.name AS source_name, end.uid AS target_id, end.name AS target_name, rel.uid AS edge_id, rel.context AS context_id,
    // rel.statement AS statement_id, rel.weight AS weight;
    ...
    if (contexts.length > 0 && contexts[0]) {
        for (var u = 0; u < contexts.length; ++u) {
            ...
            querynodes +=
                " db.index.fulltext.queryRelationships('to_context_fulltext','context:" +
                Instruments.findInArray(contexts_map, contexts[u]) +
                "') YIELD relationship AS rel, startNode AS start, endNode AS end "
            querynodes +=
                view_filter +
                ...
                " db.index.fulltext.queryRelationships('to_context_fulltext','context:" +
                Instruments.findInArray(contexts_map, contexts[u]) +
                "') YIELD relationship AS rel, startNode AS start, endNode AS end WITH rel, start, end ' " +
    ...
    // This is when we view all the (publicly) available graphs of a person...
        ...
        // We have more than 3 contexts, so the query will be a bit long, therefore we design it differently
            ...
            if (
                ...
                querynodes =
                    "CALL db.index.fulltext.queryRelationships('to_context_fulltext','context:(" +
                    filter_context +
                    ")') YIELD relationship AS rel, startNode AS start, endNode AS end " +
                    view_filter +
                    ' WITH DISTINCT COLLECT(rel) AS rs, start, end ' +
    ...
    if (showcontexts) {
        if (
            ...
            show_contexts =
                " UNION CALL db.index.fulltext.queryRelationships('at_user_fulltext','user:" +
                receiver +
                "') YIELD relationship AS rel, startNode AS start, endNode AS end " +
                view_filter +
                'WITH DISTINCT rel, start, end ' +
    ...
    dbneo.cypherQuery(querynodes, function(err, nodes) {
        ...
        for (var i = 0; i < nodes_object.length; i++) {
            if (Instruments.findInArray(contexts_map, nodes_object[i][5], 1)) {
                // Set index for source and target to -1
                var indexsource = -1
                var indextarget = -1

                // Reiterate through the sorted array of search results
        ...
        for (var i = 0; i < nodes_object.length; i++) {
            if (Instruments.findInArray(contexts_map, nodes_object[i][5], 1)) {
                ...
                if (sourcein && targetin) {
                    ...
                    if (!nodes_object[i][4]) {
                        nodes_object[i][4] = 'context' + uuidv1()
                    }
                    ...
                    if (edges_added[nodes_object[i][0] + '-' + nodes_object[i][2]]) {     
                        ...
                        if (edges_added[nodes_object[i][0] + '-' + nodes_object[i][2]].context_matrix[current_context_name]) {
                            ...
                            edges_added[nodes_object[i][0] + '-' + nodes_object[i][2]].context_matrix[current_context_name][nodes_object[i][6]] = parseInt(nodes_object[i][7]);
                            edges_added[nodes_object[i][0] + '-' + nodes_object[i][2]].weight += parseInt(nodes_object[i][7]);
                        }
